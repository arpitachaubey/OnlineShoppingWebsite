...
<a href="welcomecustomer.jsp" style="text-decoration: none">
	...
	<center>
		...
		<a href="yourorder.jsp"><i class="fa fa-first-order"></i> Orders</a> 
			...
			<a class="active" href="yourcard.jsp"><i class="fa fa-credit-card"></i> Cards</a>
...
<a href='logout.jsp'>Log out</a>
			...
			<br> 
			<h2 align="center"><font><strong>Card list</strong></font></h2><br>
...
<td><b>Action</b></td>
			...
			String email=myCookie.getValue().toString();
				...
				if (Conn != null) {
					try {
						ps = Conn.prepareStatement("select * from cards where email='" + email + "'");
						ResultSet rs = ps.executeQuery();

						while (rs.next()) {
							%>
							<tr bgcolor="#DEB887";>
			<input type="hidden" vlaue="<%=rs.getString("cardid")%>">
			<input type="hidden" vlaue="<%=rs.getString("cvv")%>">
							<td><%=rs.getString("holdername") %></td>
							<td><%=rs.getString("cardno") %></td>
							<td><%=rs.getString("expdate") %></td>
							
							<td>
							<a href="deletecard.jsp?cardid=<%=rs.getString("cardid") %>">Delete</a></td>
							</tr>
...
<h2><a href="addcard.jsp">Add new card details</a></h2>
